...
function sequence() {
    Promise.each(countries, country => {
        ...
        .catch(err => console.error(`Error: no data found for ${country}`))
    })
    .then(() => console.log('all done! 🏋️'))
...
//--------------------------------------------------------
function parallel() {
    Promise.map(countries, (country, index) => {
        return Promise.delay(index * 2000)
        .then(() => getCountryPopulation(country))
        .then(population => console.log(`population of ${country} is ${population}`))
        .catch(err => console.error(`Error: no data found for ${country}`))
    })
    .then(() => console.log('all done! 🏋️'))
